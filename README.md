...
| `shadowDistance` | `number \| "auto"` (px) | `"auto"` | Distance from the grid plane to the backdrop that catches the cast shadow. `"auto"` sizes it to clear the deepest loaded model (across any rotation) plus arrangement jitter, so objects never visually clip through the backdrop. Pass a number to pin an exact distance instead - too small a value can let an object clip through, which may be an intentional stylized choice. |
...
| `rotationOrder` | `"XYZ" \| "XZY" \| "YXZ" \| "YZX" \| "ZXY" \| "ZYX"` | `"XYZ"` | The order the three `rotation` axes combine in, only relevant once 2+ axes are non-zero at once. Axes apply right-to-left: the *last* letter is applied first (innermost, most easily distorted by the other two); the *first* letter is applied last (outermost), so it always spins around its own true world axis regardless of what the other two are doing - fixed or `"random"`. With the default camera (looking down -Z, Y up), X is pitch, Y is yaw, Z is roll/tilt-toward-viewer - pick `"ZYX"` to keep a Z tilt always reading clean while Y (or X) varies randomly. |
